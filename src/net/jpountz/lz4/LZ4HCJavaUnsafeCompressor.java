...
/*  48 */       int v = UnsafeUtils.readInt(bytes, off);
...
/*  67 */       int v = UnsafeUtils.readInt(bytes, off);
/*  68 */       addHash(v, off);
/*     */     }
/*     */     
...
/* 112 */         if (LZ4UnsafeUtils.readIntEquals(buf, ref, off)) {
/* 113 */           delta = off - ref;
/* 114 */           repl = match.len = 4 + LZ4UnsafeUtils.commonBytes(buf, ref + 4, off + 4, matchLimit);
/* 115 */           match.ref = ref;
/*     */         } 
/* 117 */         ref = next(ref);
/*     */       } 
/*     */       
/* 120 */       for (int i = 0; i < LZ4HCJavaUnsafeCompressor.this.maxAttempts && 
/* 121 */         ref >= Math.max(this.base, off - 65536 + 1) && ref <= off; i++) {
/*     */ 
/*     */         
/* 124 */         if (LZ4UnsafeUtils.readIntEquals(buf, ref, off)) {
/* 125 */           int matchLen = 4 + LZ4UnsafeUtils.commonBytes(buf, ref + 4, off + 4, matchLimit);
...
/* 143 */           this.hashTable[LZ4Utils.hashHC(UnsafeUtils.readInt(buf, ptr))] = ptr;
/* 144 */           ptr++;
/* 145 */           if (ptr >= end) {
...
/* 159 */       for (int i = 0; i < LZ4HCJavaUnsafeCompressor.this.maxAttempts && 
/* 160 */         ref >= Math.max(this.base, off - 65536 + 1) && ref <= off; i++) {
/*     */ 
/*     */         
/* 163 */         if (LZ4UnsafeUtils.readIntEquals(buf, ref, off)) {
/* 164 */           int matchLenForward = 4 + LZ4UnsafeUtils.commonBytes(buf, ref + 4, off + 4, matchLimit);
/* 165 */           int matchLenBackward = LZ4UnsafeUtils.commonBytesBackward(buf, ref, off, this.base, startLimit);
...
/*     */       } 
/*     */       
/* 199 */       for (int i = 0; i < LZ4HCJavaUnsafeCompressor.this.maxAttempts && 
...
/* 238 */       for (int i = 0; i < LZ4HCJavaUnsafeCompressor.this.maxAttempts && 
...
/* 265 */     UnsafeUtils.checkRange(src, srcOff, srcLen);
/* 266 */     UnsafeUtils.checkRange(dest, destOff, maxDestLen);
/*     */     
...
/*     */ 
/*     */           
/* 299 */           dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len, dest, dOff, destEnd);
...
/*     */             }
/*     */             
/* 339 */             dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len, dest, dOff, destEnd);
/* 340 */             anchor = sOff = match1.end();
/*     */             
/* 342 */             dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match2.start, match2.ref, match2.len, dest, dOff, destEnd);
...
/*     */               } 
/*     */               
/* 357 */               dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len, dest, dOff, destEnd);
...
/*     */           }
/*     */           
/* 386 */           dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len, dest, dOff, destEnd);
...
/*     */ 
/*     */     
/* 399 */     dOff = LZ4UnsafeUtils.lastLiterals(src, anchor, srcEnd - anchor, dest, dOff, destEnd);
