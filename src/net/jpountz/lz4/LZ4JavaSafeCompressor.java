...
/*     */           }
/*     */           
/*  55 */           int h = LZ4Utils.hash64k(SafeUtils.readInt(src, sOff));
/*  56 */           int ref = srcOff + SafeUtils.readShort(hashTable, h);
/*  57 */           SafeUtils.writeShort(hashTable, h, sOff - srcOff);
/*  58 */           if (LZ4SafeUtils.readIntEquals(src, ref, sOff)) {
/*     */ 
/*     */             
/*  61 */             int excess = LZ4SafeUtils.commonBytesBackward(src, ref, sOff, srcOff, anchor);
/*  62 */             sOff -= excess;
/*  63 */             ref -= excess;
/*     */ 
/*     */             
...
/*     */             
/*  75 */             if (runLen >= 15) {
/*  76 */               SafeUtils.writeByte(dest, tokenOff, 240);
/*  77 */               dOff = LZ4SafeUtils.writeLen(runLen - 15, dest, dOff);
/*     */             } else {
/*  79 */               SafeUtils.writeByte(dest, tokenOff, runLen << 4);
/*     */             } 
/*     */ 
/*     */             
/*  83 */             LZ4SafeUtils.wildArraycopy(src, anchor, dest, dOff, runLen);
/*  84 */             dOff += runLen;
/*     */ 
/*     */             
/*     */             while (true)
/*  88 */             { SafeUtils.writeShortLE(dest, dOff, (short)(sOff - ref));
/*  89 */               dOff += 2;
/*     */ 
/*     */               
/*  92 */               sOff += 4;
/*  93 */               ref += 4;
/*  94 */               int matchLen = LZ4SafeUtils.commonBytes(src, ref, sOff, srcLimit);
...
/*     */               
/* 101 */               if (matchLen >= 15) {
/* 102 */                 SafeUtils.writeByte(dest, tokenOff, SafeUtils.readByte(dest, tokenOff) | 0xF);
/* 103 */                 dOff = LZ4SafeUtils.writeLen(matchLen - 15, dest, dOff);
/*     */               } else {
/* 105 */                 SafeUtils.writeByte(dest, tokenOff, SafeUtils.readByte(dest, tokenOff) | matchLen);
/*     */               } 
/*     */ 
/*     */               
/* 109 */               if (sOff > mflimit) {
...
/*     */               } 
/*     */               
/* 115 */               SafeUtils.writeShort(hashTable, LZ4Utils.hash64k(SafeUtils.readInt(src, sOff - 2)), sOff - 2 - srcOff);
/*     */ 
/*     */               
/* 118 */               int i = LZ4Utils.hash64k(SafeUtils.readInt(src, sOff));
/* 119 */               ref = srcOff + SafeUtils.readShort(hashTable, i);
/* 120 */               SafeUtils.writeShort(hashTable, i, sOff - srcOff);
/*     */               
/* 122 */               if (!LZ4SafeUtils.readIntEquals(src, sOff, ref))
/*     */               
/*     */               { 
/*     */ 
/*     */ 
...
/*     */ 
/*     */                 
/* 131 */                 anchor = sOff++; continue; }  tokenOff = dOff++; SafeUtils.writeByte(dest, tokenOff, 0); }  break;
/*     */           } 
/*     */         }  break;
/*     */       } 
/* 135 */     }  dOff = LZ4SafeUtils.lastLiterals(src, anchor, srcEnd - anchor, dest, dOff, destEnd);
...
/* 142 */     SafeUtils.checkRange(src, srcOff, srcLen);
/* 143 */     SafeUtils.checkRange(dest, destOff, maxDestLen);
...
/*     */         }
/*     */         
/* 179 */         int h = LZ4Utils.hash(SafeUtils.readInt(src, sOff));
/* 180 */         int ref = SafeUtils.readInt(hashTable, h);
/* 181 */         int back = sOff - ref;
/* 182 */         SafeUtils.writeInt(hashTable, h, sOff);
/* 183 */         if (back < 65536 && LZ4SafeUtils.readIntEquals(src, ref, sOff)) {
/*     */ 
/*     */           
/* 186 */           int excess = LZ4SafeUtils.commonBytesBackward(src, ref, sOff, srcOff, anchor);
/* 187 */           sOff -= excess;
/* 188 */           ref -= excess;
/*     */ 
/*     */           
...
/*     */           
/* 200 */           if (runLen >= 15) {
/* 201 */             SafeUtils.writeByte(dest, tokenOff, 240);
/* 202 */             dOff = LZ4SafeUtils.writeLen(runLen - 15, dest, dOff);
/*     */           } else {
/* 204 */             SafeUtils.writeByte(dest, tokenOff, runLen << 4);
/*     */           } 
/*     */ 
/*     */           
/* 208 */           LZ4SafeUtils.wildArraycopy(src, anchor, dest, dOff, runLen);
/* 209 */           dOff += runLen;
/*     */ 
/*     */           
/*     */           while (true)
/* 213 */           { SafeUtils.writeShortLE(dest, dOff, back);
/* 214 */             dOff += 2;
/*     */ 
/*     */             
/* 217 */             sOff += 4;
/* 218 */             int matchLen = LZ4SafeUtils.commonBytes(src, ref + 4, sOff, srcLimit);
...
/*     */             
/* 225 */             if (matchLen >= 15) {
/* 226 */               SafeUtils.writeByte(dest, tokenOff, SafeUtils.readByte(dest, tokenOff) | 0xF);
/* 227 */               dOff = LZ4SafeUtils.writeLen(matchLen - 15, dest, dOff);
/*     */             } else {
/* 229 */               SafeUtils.writeByte(dest, tokenOff, SafeUtils.readByte(dest, tokenOff) | matchLen);
/*     */             } 
/*     */ 
/*     */             
/* 233 */             if (sOff > mflimit) {
...
/*     */             } 
/*     */             
/* 239 */             SafeUtils.writeInt(hashTable, LZ4Utils.hash(SafeUtils.readInt(src, sOff - 2)), sOff - 2);
/*     */ 
/*     */             
/* 242 */             int i = LZ4Utils.hash(SafeUtils.readInt(src, sOff));
/* 243 */             ref = SafeUtils.readInt(hashTable, i);
/* 244 */             SafeUtils.writeInt(hashTable, i, sOff);
/* 245 */             back = sOff - ref;
/*     */             
/* 247 */             if (back < 65536) { if (!LZ4SafeUtils.readIntEquals(src, ref, sOff)) {
/*     */                 continue label43;
/*     */               }
/*     */               
/* 251 */               tokenOff = dOff++;
/* 252 */               SafeUtils.writeByte(dest, tokenOff, 0);
/*     */               
/*     */               continue; }
/*     */             
...
/*     */         } 
/*     */       }  break;
/* 259 */     }  dOff = LZ4SafeUtils.lastLiterals(src, anchor, srcEnd - anchor, dest, dOff, destEnd);
...
/* 299 */           int ref = srcOff + SafeUtils.readShort(hashTable, h);
/* 300 */           SafeUtils.writeShort(hashTable, h, sOff - srcOff);
...
/*     */               } 
/*     */               
/* 358 */               SafeUtils.writeShort(hashTable, LZ4Utils.hash64k(ByteBufferUtils.readInt(src, sOff - 2)), sOff - 2 - srcOff);
/*     */ 
/*     */               
/* 361 */               int i = LZ4Utils.hash64k(ByteBufferUtils.readInt(src, sOff));
/* 362 */               ref = srcOff + SafeUtils.readShort(hashTable, i);
/* 363 */               SafeUtils.writeShort(hashTable, i, sOff - srcOff);
/*     */               
...
/* 429 */         int ref = SafeUtils.readInt(hashTable, h);
/* 430 */         int back = sOff - ref;
/* 431 */         SafeUtils.writeInt(hashTable, h, sOff);
...
/*     */             } 
/*     */             
/* 488 */             SafeUtils.writeInt(hashTable, LZ4Utils.hash(ByteBufferUtils.readInt(src, sOff - 2)), sOff - 2);
/*     */ 
/*     */             
/* 491 */             int i = LZ4Utils.hash(ByteBufferUtils.readInt(src, sOff));
/* 492 */             ref = SafeUtils.readInt(hashTable, i);
/* 493 */             SafeUtils.writeInt(hashTable, i, sOff);
/* 494 */             back = sOff - ref;
/*     */             
