...
/*     */ {
/*  17 */   public static final LZ4SafeDecompressor INSTANCE = new LZ4JavaUnsafeSafeDecompressor();
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*  24 */     UnsafeUtils.checkRange(src, srcOff, srcLen);
/*  25 */     UnsafeUtils.checkRange(dest, destOff, destLen);
/*     */     
/*  27 */     if (destLen == 0) {
/*  28 */       if (srcLen != 1 || UnsafeUtils.readByte(src, srcOff) != 0) {
...
/*     */     
/*     */     while (true) {
/*  43 */       int token = UnsafeUtils.readByte(src, sOff) & 0xFF;
/*  44 */       sOff++;
/*     */ 
/*     */       
...
/*  48 */       if (literalLen == 15) {
/*  49 */         byte len = -1;
/*  50 */         while (sOff < srcEnd && (len = UnsafeUtils.readByte(src, sOff++)) == -1) {
/*  51 */           literalLen += 255;
/*     */         }
...
/*     */         }
/*     */         
/*  65 */         LZ4UnsafeUtils.safeArraycopy(src, sOff, dest, dOff, literalLen);
/*  66 */         sOff += literalLen;
...
/*     */       } 
/*     */       
/*  72 */       LZ4UnsafeUtils.wildArraycopy(src, sOff, dest, dOff, literalLen);
/*  73 */       sOff += literalLen;
/*  74 */       dOff = literalCopyEnd;
/*     */ 
/*     */       
/*  77 */       int matchDec = UnsafeUtils.readShortLE(src, sOff);
/*  78 */       sOff += 2;
...
/*  86 */       if (matchLen == 15) {
/*  87 */         byte len = -1;
/*  88 */         while (sOff < srcEnd && (len = UnsafeUtils.readByte(src, sOff++)) == -1) {
/*  89 */           matchLen += 255;
/*     */         }
...
/*     */         }
/* 101 */         LZ4UnsafeUtils.safeIncrementalCopy(dest, matchOff, dOff, matchLen);
/*     */       } else {
/* 103 */         LZ4UnsafeUtils.wildIncrementalCopy(dest, matchOff, dOff, matchCopyEnd);
/*     */       } 
...
/* Location:              E:\NMSSE_Source\NMSSaveEditor.jar!\net\jpountz\lz4\LZ4JavaUnsafeSafeDecompressor.class
