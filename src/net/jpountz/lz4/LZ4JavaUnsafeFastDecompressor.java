   ...
   public int decompress(byte[] src, int srcOff, byte[] dest, int destOff, int destLen) {
      ...
      if (destLen == 0) {
         ...
         while(true) {
            ...
            if (literalLen == 15) {
               byte len;
               for(boolean var11 = true; (len = UnsafeUtils.readByte(src, sOff++)) == -1; literalLen += 255) {
               }
            ...
            if (matchLen == 15) {
               byte len;
               for(boolean var15 = true; (len = UnsafeUtils.readByte(src, sOff++)) == -1; matchLen += 255) {
               }
      ...
      if (src.hasArray() && dest.hasArray()) {
         ...
         if (destLen == 0) {
            ...
            while(true) {
               ...
               if (literalLen == 15) {
                  byte len;
                  for(boolean var11 = true; (len = ByteBufferUtils.readByte(src, sOff++)) == -1; literalLen += 255) {
                  }
               ...
               if (matchLen == 15) {
                  byte len;
                  for(boolean var15 = true; (len = ByteBufferUtils.readByte(src, sOff++)) == -1; matchLen += 255) {
                  }
